...
- **Módulo de bodega** (fase 1, se activa por empresa desde súper admin): inicio con Ingreso, Salida, Inventario y Dashboard; ingresos con OC, guía y fotos; salidas con quién retira y quién despacha; SOH mantenido por la base en `skus.stock_sistema`; movimientos con anulación y pendientes de aprobación; stock actual; listas de proveedores y personas; cola sin conexión. Fase 2: ajustes de stock con aprobación (desde Stock o desde Reconteo con la cantidad contada; el operador propone y el administrador aprueba) y kardex por SKU con saldo corrido desde Stock, Movimientos y Buscar; crear un SKU nuevo desde el Ingreso; comprobante PDF de ingreso y salida; exportación a Excel de movimientos y stock; tarjeta de bodega en el Dashboard. Fase 3: reportes de consumo por persona, área, destino o material e ingresos por proveedor, valorización del stock y stock mínimo por material con filtro de reposición; botón para registrar la apertura de stock cuando el módulo se activa sobre materiales ya cargados. El ingreso captura el costo unitario por línea (monto del documento) y cada material puede clasificarse por tipo (EPP, repuesto, sustancia peligrosa, componente, otros), usable como agrupación en los reportes. Diseño en `docs/DISENO-MODULO-BODEGA.md`.
